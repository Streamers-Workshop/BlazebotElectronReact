...
$(document).ready(() => {
  ...
  $.each(arrayEffect, (key, value) => {
    selectEffect.append("<option value=" + key + ">" + value[0] + "</option>");
  });

  selectEffect.on("change", (select) => {
    var urlDisplay = $("#txtURL").val(
      arrayEffect[select.currentTarget.value][1]
    );

    switch (select.currentTarget.value) {
      case "1":
        $("#displayEffect").html(
